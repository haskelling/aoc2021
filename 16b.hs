...
eval (Lit _ x) = x
eval (Op _ t is) = oper t $ map eval is

oper 0 xs = sum xs
oper 1 xs = product xs
oper 2 xs = minimum xs
oper 3 xs = maximum xs
oper 5 [x1, x2] = if x1 > x2 then 1 else 0
oper 6 [x1, x2] = if x1 < x2 then 1 else 0
oper 7 [x1, x2] = if x1 == x2 then 1 else 0

f = eval . fromRight (Lit 0 0) . P.parse instr "" . concatMap d

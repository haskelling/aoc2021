...
f ts = maximum $ map (\(x, y) -> manhattan3 (x - y)) $ allpairs scans
  where
    tsm = M.fromList ts
    ns = map fst ts
    trans = g tsm [head ns] (tail ns) (M.singleton (head ns) id) (S.fromList $ snd $ head ts)
    scans = map (($ 0) . (trans M.!)) ns
    allpairs [] = []
    allpairs (s:ss) = map (s,) ss ++ allpairs ss
